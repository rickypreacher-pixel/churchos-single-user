...
fp = r"C:\Users\Admin\OneDrive\Desktop\ntcc-church-app-2026\src\App.tsx"
                            ...
                            <div style={{borderTop:"0.5px solid "+BR}}>
                              {PLANNER_SERVICES.includes(evt.name)&&(
                                <div style={{display:"flex",borderBottom:"0.5px solid "+BR,background:BG,flexShrink:0}}>
                                  {(["checkin","planner"] as string[]).map((t:string)=>(
...
if old in src:
    ...
    if old_crlf in src:
        src = src.replace(old_crlf, new, 1)
        print("C3 OK (CRLF)")
    else:
        print("C3 NOT FOUND")
        ...
        if idx >= 0:
            print(f"isSel found at char {idx}, showing next 200 chars:")
